...
const MEMBERS = {
  ...
  rubina: { name: "Rubina", role: "The Daydreamer", emoji: "🌿", tagline: "pluto in aquarius", color: MEMBER_COLORS.rubina },
};

const COMMUNITY_POSTS = [
  { id: 1, author: "the stamps", avatar: "✦", badge: "band", content: "studio day 47. we wrote something at 2am that made all three of us cry.", likes: 1203, comments: 287, time: "3h ago" },
  { id: 2, author: "wavesandwonder", avatar: "w", badge: null, content: "just played holy verse for my mum and she cried.", likes: 89, comments: 23, time: "5h ago" },
  ...
  { id: 4, author: "fremantle.kid", avatar: "f", badge: "og", content: "been following since the nannup days.", likes: 156, comments: 41, time: "1d ago" },
...
const MEMBER_POSTS = {
  sofia: [
    { id: "s1", content: "been listening to lizzy mcalpine and the paper kites.", time: "2h ago", likes: 234, comments: 45 },
    { id: "s2", content: "journal pages from the writing session.", time: "1d ago", likes: 567, comments: 89, exclusive: true },
    { id: "s3", content: "the song tells me what i meant.", time: "3d ago", likes: 891, comments: 132 },
  ],
  scarlett: [
    { id: "sc1", content: "found a tiny venue in fitzroy with incredible acoustics.", time: "4h ago", likes: 189, comments: 34 },
    { id: "sc2", content: "DEMO DROP: rough guitar loop.", time: "2d ago", likes: 445, comments: 167, exclusive: true },
    { id: "sc3", content: "joni mitchell changed everything for me.", time: "4d ago", likes: 623, comments: 78 },
  ],
  rubina: [
    { id: "r1", content: "pluto is in aquarius. 2026 is the year of the stamp.", time: "6h ago", likes: 312, comments: 67 },
    { id: "r2", content: "PLAYLIST: rubinas sunday slow burn - updated weekly.", time: "1d ago", likes: 534, comments: 45 },
    { id: "r3", content: "being three people in a band means you are never alone with a feeling.", time: "5d ago", likes: 789, comments: 156 },
  ],
};
...
const STAMP_LEVELS = [
  { name: "First Press", desc: "Join the community", stamps: 0, unlocked: true },
  { name: "B-Side", desc: "10 posts or comments", stamps: 50, unlocked: true },
  { name: "Deep Cut", desc: "Attend a live show", stamps: 150, unlocked: false },
  { name: "Inner Sleeve", desc: "6 months active", stamps: 300, unlocked: false },
  { name: "Stamped", desc: "Unlock everything", stamps: 500, unlocked: false },
];
  ...
  return (
    <div style={{ background: isEx ? DARK_CREAM : OFF_WHITE, borderRadius: 3, padding: "18px 20px", marginBottom: 10, border: "1px solid " + (isEx ? WARM_GOLD + "44" : "#E8E2D8") }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        <div style={{ width: 34, height: 34, borderRadius: 3, background: isBand ? INK : "#eee", color: isBand ? CREAM : SLATE, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13 }}>{post.avatar}</div>
        <div>
          <div style={{ fontWeight: 600, fontSize: 13, color: INK }}>{post.author}</div>
          <div style={{ fontSize: 10, color: SLATE }}>{post.time}</div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 18 }}>
        <button onClick={() => setLiked(!liked)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 12, color: liked ? RUST : SLATE, padding: 0 }}>
          {liked ? "loved" : "love"} {post.likes + (liked ? 1 : 0)}
        </button>
        <span style={{ fontSize: 12, color: SLATE }}>reply {post.comments}</span>
      </div>
    </div>
  );
}
    ...
    <div>
      <div style={{ background: m.color.bg, borderRadius: 3, padding: "22px 20px", marginBottom: 14, borderLeft: "3px solid " + m.color.primary }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            <div style={{ fontFamily: "serif", fontSize: 22, fontWeight: 900 }}>{m.name}</div>
...
export default function StampsLand() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: CREAM }}>
      <div style={{ position: "sticky", top: 0, zIndex: 100, background: CREAM, borderBottom: "1px solid " + DARK_CREAM }}>
        <div style={{ maxWidth: 480, margin: "0 auto", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontFamily: "serif", fontSize: 24, fontWeight: 900 }}>the stamps</div>
          <div style={{ fontSize: 12, background: DARK_CREAM, padding: "5px 12px", borderRadius: 2 }}>stamps: {userStamps}</div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 16px 100px" }}>
        {showWelcome && mainTab === "feed" && (
          <div style={{ background: INK, borderRadius: 3, padding: "24px 22px", margin: "14px 0" }}>
            <button onClick={() => setShowWelcome(false)} style={{ float: "right", background: "none", border: "none", color: CREAM, cursor: "pointer", fontSize: 16 }}>x</button>
            <div style={{ fontFamily: "serif", fontSize: 28, fontWeight: 900, color: CREAM, marginBottom: 10 }}>welcome to stamps land</div>
            <p style={{ fontSize: 13, color: CREAM, lineHeight: 1.6, marginBottom: 16 }}>you are in. earn stamps by being here.</p>
        ...
        {mainTab === "feed" && (
          <div>
            <div style={{ display: "flex", borderBottom: "1px solid " + DARK_CREAM, marginTop: 10, marginBottom: 14 }}>
              <button onClick={() => setFeedView("community")} style={{ flex: 1, padding: "12px 6px 10px", background: "none", border: "none", borderBottom: feedView === "community" ? "2px solid " + INK : "2px solid transparent", cursor: "pointer", fontSize: 11, fontWeight: feedView === "community" ? 700 : 400 }}>everyone</button>
              {Object.keys(MEMBERS).map(key => {
                const m = MEMBERS[key];
                return (
                  <button key={key} onClick={() => setFeedView(key)} style={{ flex: 1, padding: "12px 6px 10px", background: "none", border: "none", borderBottom: feedView === key ? "2px solid " + m.color.primary : "2px solid transparent", cursor: "pointer", fontSize: 11, fontWeight: feedView === key ? 700 : 400 }}>
                    {m.emoji} {m.name}
                  </button>
                );
              })}
        ...
        {mainTab === "shows" && (
          <div style={{ paddingTop: 14 }}>
            <div style={{ background: INK, borderRadius: 3, padding: "28px 22px", marginBottom: 16 }}>
              <div style={{ fontFamily: "serif", fontSize: 34, fontWeight: 900, color: CREAM }}>the stamps</div>
              <div style={{ fontSize: 13, color: CREAM, marginTop: 6 }}>AU - EU - UK</div>
            </div>
            {TOUR_SECTIONS.map(section => (
              <div key={section.region} style={{ marginBottom: 10 }}>
                <button onClick={() => setExpandedRegion(expandedRegion === section.region ? null : section.region)} style={{ width: "100%", background: expandedRegion === section.region ? INK : OFF_WHITE, border: "1px solid #E8E2D8", borderRadius: 3, padding: "14px 18px", display: "flex", justifyContent: "space-between", cursor: "pointer" }}>
                  ...
                  <span style={{ fontSize: 11, color: expandedRegion === section.region ? CREAM : SLATE }}>{section.dates.length} shows</span>
                </button>
        ...
        {mainTab === "stamps" && (
          <div style={{ paddingTop: 14 }}>
            <div style={{ background: INK, borderRadius: 3, padding: "30px 22px", textAlign: "center", marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 10, color: CREAM, marginTop: 6 }}>stamps collected</div>
            </div>
            {STAMP_LEVELS.map(level => (
              <div key={level.name} style={{ background: OFF_WHITE, borderRadius: 3, padding: "14px 18px", marginBottom: 6, border: "1px solid #E8E2D8", display: "flex", alignItems: "center", gap: 14, opacity: level.unlocked ? 1 : 0.5 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: INK }}>{level.name}</div>
                  <div style={{ fontSize: 10, color: SLATE }}>{level.desc}</div>
                </div>
                <div style={{ fontSize: 11, color: SLATE }}>{level.stamps > 0 ? level.stamps + " stamps" : "done"}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {mainTab === "you" && (
          <div style={{ paddingTop: 14 }}>
            <div style={{ background: OFF_WHITE, borderRadius: 3, padding: "28px 22px", textAlign: "center", border: "1px solid #E8E2D8" }}>
              <div style={{ fontFamily: "serif", fontSize: 22, fontWeight: 900 }}>newstamp</div>
              <div style={{ fontSize: 10, color: SLATE, marginTop: 4 }}>member since march 2026</div>
            </div>
          </div>
        )}
      </div>

      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 100, background: OFF_WHITE, borderTop: "1px solid " + DARK_CREAM }}>
        <div style={{ maxWidth: 480, margin: "0 auto", display: "flex", justifyContent: "space-around", padding: "10px 0 24px" }}>
          {mainTabs.map(tab => (
            <button key={tab.id} onClick={() => setMainTab(tab.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: "8px 18px" }}>
              <span style={{ fontSize: 12, color: mainTab === tab.id ? INK : SLATE }}>{tab.label}</span>
            </button>
          ))}
        </div>
      </div>
